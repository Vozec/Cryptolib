...
class rsa_system():
	...
	def full(self):
		return self.e != None and self.p!= None and self.q!= None and \
		self.n != None and self.d!= None  and self.phi != None
	...
	def update(self,e=None,q=None,p=None,n=None,d=None,phi=None,c=None):
		...
		elif n:self.n = [n] if type(n) != list else n
		elif c:self.c = [c] if type(c) != list else c
		elif p:self.p = p
		elif q:self.q = q
		elif d:self.d = d
	...
	def decrypt(self,m):
		...
		return long_to_bytes(pow(m,self.d,self.n[0]))
	################################
